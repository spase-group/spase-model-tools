...
public class XML {
	...
	/**
	 * Parses a file containing XML into its constitute elments. The path and
	 * name of the file are passed to the method which is opened and parsed.
	 * 
	 * @param pathName
	 *            the fully qualified path and name of the file to parse.
	 * 
	 * @return <code>true</code> if the file could be opened; <code>false</code>
	 *         otherwise.
	 * @since 1.0
	 */
	public boolean parseXML(String pathName) throws Exception {
	...
	/**
	 * Parses a file containing XML into its constitute elments. The file to
	 * parse must be previously opened and a InputStream pointing to the file is
	 * passed.
	 * 
	 * @param stream
	 *            a connection to a pre-opened file.
	 * 
	 * @return <code>true</code> if the file could be read; <code>false</code>
	 *         otherwise.
	 * @since 1.0
	 */
	...
	/**
	 * Generates an XML representation of the label and stream it to the print
	 * stream.
	 * 
	 * @param out
	 ...
	 * @since 1.0
	 */
	public void dumpData(PrintStream out) throws Exception {
		Node node;
		DOMSource source = new DOMSource(mDocument);
	...
	public void dumpNode(PrintStream out, Node node) {
		Node child;
		Node sibling;
		out.println(node.getNodeType() + ": " + node.getNodeName() + ": "
				+ node.getNodeValue());
