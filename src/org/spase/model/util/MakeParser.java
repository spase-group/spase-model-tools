...
public class MakeParser extends Query {
	...
	private String mOverview = "Creates a set of Java classes to create a parser that matches a schema.\n"
			 ;
	...
	public void writeHeader(String name, String version, boolean isResource)
		...
		printLine("package spase.parser%param%;", mExtension+ver);
		printLine("");
		printLine("import org.w3c.dom.Node;");
		printLine("import java.util.ArrayList;");
		printLine("import java.util.Iterator;");
		if( ! mDatabase.equals("spase-model.db")) {
			printLine("import spase.parser221.*;");
		}
		printLine("");
		printLine("/**");
	...
	public void writeFooter(String name) throws Exception {
		printLine("}");
	}

	public void writeVariables(ArrayList list) throws Exception {
		String elemName;
		String buffer;
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isMultiple(buffer)) {
				if (isContainer(getTermName(buffer))) {
					printLine(
							"   private ArrayList<%param%> m%param% = new ArrayList<%param%>();",
							elemName);
				} else {
					printLine(
							"   private ArrayList<String> m%param% = new ArrayList<String>();",
							elemName);
				}
			} else {
				if (isContainer(getTermName(buffer))) {
					printLine("   private %param% m%param% = new %param%();",
							elemName);
				} else {
	...
	public void writeNodeMethods(String objectName, ArrayList list)
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isContainer(getTermName(tokenName))) {
				if (isMultiple(tokenName)) {
					printLine("      m%param%.add(new %param%(true));",
							elemName);
				} else {
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isContainer(getTermName(tokenName))) {
				if (isMultiple(tokenName)) {
					printLine(
							"      for(Iterator i = m%param%.iterator(); i.hasNext();) { ((%param%) i.next()).makeSkeletonNodes(); }",
							elemName);
				} else {
				...
				if (isMultiple(tokenName)) {
					printLine(
							"      if(m%param%.isEmpty()) m%param%.add(\"\");",
							elemName);
				}
			}
		}
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isContainer(getTermName(tokenName))) {
				if (isMultiple(tokenName)) {
					printLine(
							"      if(isMatch(nodeName, \"%param%\")) return m%param%.get(nodeIndex);",
							elemName);
				} else {
					printLine(
							"      if(isMatch(nodeName, \"%param%\")) return m%param%;",
							elemName);
				}
			}
		}
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isMultiple(tokenName)) {
				if (isContainer(getTermName(tokenName))) {
					printLine(
							"      if(isMatch(item, \"%param%\")) m%param%.add(new %param%());",
							elemName);
				} else {
					printLine(
							"      if(isMatch(item, \"%param%\")) m%param%.add(\"\");",
							elemName);
				}
			}
		}
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isMultiple(tokenName)) {
				printLine(
						"      if(isInList(item, \"%param%\", m%param%, index)) m%param%.remove(index);",
						elemName);
			}
		}
		printLine("   }");
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isMultiple(tokenName)) {
				if (isContainer(getTermName(tokenName))) {
					...
					printLine(
							"   	buffer += getTaggedList(n+1, \"%param%\", m%param%, inUseOnly);",
							elemName);
				}
			} else {
				if (isContainer(getTermName(tokenName))) {
					printLine(
							"   	buffer += m%param%.getXMLDocument(n+1, elemPath, -1, inUseOnly);",
							elemName);
				} else {
					printLine(
							"   	buffer += getTaggedValue(n+1, \"%param%\", m%param%, inUseOnly);",
							elemName);
				}
			}
		}
	...
	public void writeSetGet(ArrayList list) throws Exception {
		String tokenName;
		String elemName;
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isMultiple(tokenName)) {
				...
				if (isContainer(getTermName(tokenName))) {
					printLine(
							"   public void set%param%(Node node) throws Exception { %param% i = new %param%(); i.processNode(node); m%param%.add(i);}",
							elemName);
					printLine(
							"   public ArrayList<%param%> get%param%() { return m%param%; }",
							elemName);
				} else {
					printLine(
							"   public void set%param%(String value) { m%param%.add(value); }",
							elemName);
					printLine(
							"   public void set%param%(String[] value) { m%param%.clear(); for(int i = 0; i < value.length; i++) { m%param%.add(value[i]); } }",
							elemName);
					printLine(
							"   public ArrayList<String> get%param%() { return m%param%; }",
							elemName);
				}
			} else {
				printLine("");
				if (isContainer(getTermName(tokenName))) {
					printLine(
							"   public void set%param%(Node node) throws Exception { m%param%.processNode(node);} ",
							elemName);
					printLine(
							"   public %param% get%param%() { return m%param%; }",
							elemName);
				} else {
					printLine(
							"   public void set%param%(String value) { m%param% = value; }",
							elemName);
					printLine(
							"   public void set%param%(String[] value) { m%param% = value[0]; }",
							elemName);
					printLine(
							"   public String get%param%() { return m%param%; }",
							elemName);
				}
			}
		}
	...
	public void writeReset(ArrayList list) {
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isMultiple(tokenName)) {
				if (isContainer(getTermName(tokenName))) {
					printLine(
							"		for(int i = 0; i < m%param%.size(); i++) m%param%.get(i).reset();",
							elemName);
				} else {
					printLine(
							"		for(int i = 0; i < m%param%.size(); i++) m%param%.set(i, \"\");",
							elemName);
				}
			} else {
				if (isContainer(getTermName(tokenName))) {
	...
	public void writeXPathPairs(String object, ArrayList list) {
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			if (isMultiple(tokenName)) {
				if (isContainer(getTermName(tokenName))) {
					printLine(
							"      list.addAll(%param%.getXPathPairs(path, m%param%));",
							elemName);
				} else {
					printLine(
							"      list.addAll(XMLParser.getXPathList(path + \"/%param%\", m%param%));",
							elemName);
				}
			} else {
				if (isContainer(getTermName(tokenName))) {
					printLine(
							"      list.addAll(m%param%.getXPathPairs(path, 0));",
							elemName);
				} else {
					printLine(
							"      list.add(new Pair<String, String>(path + \"/%param%\", m%param%));",
							elemName);
				}
			}
		}
		...
		printLine("");
		printLine(
				"	public static ArrayList<Pair> getXPathPairs(String prefix, ArrayList<%param%> list)",
				object);
		printLine("	{");
	...
	public void writeDescriptionClass(ArrayList list, String version)
		...
		for (Iterator i = list.iterator(); i.hasNext();) {
			...
			printLine(
					"             if(Util.isMatch(name, \"%param%\")) mResource.add(new %param%(sibling));",
					elemName);
		}
	...
	public void writePairClass(String version) throws Exception {
	...
	public void writeUtilClass(String version) throws Exception {
	...
	public void writeResourceClass(String version) throws Exception {
	...
	public void writeXMLParserClass(String version) throws Exception {
	...
	public void writeProjectFile(String version) throws Exception {
		String buffer;
	...
	public boolean isContainer(String term) {
		try {
			if (getElementTypeToken(term) == TypeContainer)
				return true;
		} catch (Exception e) {
		}
	...
	public boolean hasResourceHeader(String term) {
		...
		// Exceptions
		if (isMatch(term, "Person"))
			return false;
		if (isMatch(term, "Granule"))
			return false;

		for (Iterator i = mTopLevelElements.iterator(); i.hasNext();) {
			buffer = (String) i.next();
			if (isMatch(term, buffer))
				has = true;
		}
		return has;
	}
	...
	public void makeObject(String object, boolean isResource) throws Exception {
		String query;
		Statement statement;
		ResultSet resultSet;
	...
	public ArrayList getTopLevelElements() throws Exception {
		...
		list.clear();
		while (resultSet.next()) {
			if (isMatch(resultSet.getString("Element"), "Version"))
	...
	public void setExtension(String dbName) {
		mExtension = "";
		if(dbName.equals("spase-model.db")) return;	// Do nothing
		
		String base = igpp.util.Text.getFileBase(dbName);
