...
// Database access variables
	...
	public static void main(String args[])
   {
	...
	public void writeHeader(String name, String object, boolean isTop, boolean forEdit, ArrayList importList, ArrayList paramList)
		throws Exception
	{
		...
		throws Exception
	{
		...
		if(isTop) {
			...
			printLine("<table>");
			printLine("<tr><td><b>" + name + "</b></td><td width=\"100%\" align=\"right\"><input type=\"submit\" name=\"action\" Value=\"Update\" /></td></tr>");
			printLine("</table>");
			printLine("<hr/>");
			printLine("<table>");
		} else {
			printLine("<tr><td valign=\"top\"><div class=\"tab\">" + name + "</div></td><td>");
			...
			printLine("<img alt=\"Hide\" onclick=\"flipImage(this); showHide('{$elemPath}')\" src=\"open.gif\" border=\"0\" />");
 			printLine("<img valign=\"top\" alt=\"Info\" onclick=\"showDictionary('" + object + "');\" src=\"info.gif\" />");
			printLine(" </td></tr>");
	...
	public void writeEditFooter(String name, boolean isTop)
		throws Exception
	{
		if(isTop) {
		...
		throws Exception
	{
		...
		for(Iterator i = list.iterator(); i.hasNext(); ) {
			...
			case TypeTime:
				...
				if(multiple) printLine("  <xsl:if test=\"position()=1\">" + name + filler + ":</xsl:if>");
				else printLine(name + filler + ":");
				printLine("</td><td><input type=\"text\" name=\"{$elemPath}/" + elemName + "\" size=\"30\" value=\"{" + value + "}\" />" + options + "</td></tr>");
				break;
			case TypeEnumeration:
				...
				if(occur == '0' || occur == '*') {	// Allow selection of nothing
					printLine("      <xsl:if test=\"" + value + "=''\"><option selected=\"yes\"></option></xsl:if>");
					printLine("      <xsl:if test=\"" + value + "!=''\"><option></option></xsl:if>");
				}
				for(Iterator n = enumList.iterator(); n.hasNext(); ) {
					option = (String) n.next();
					printLine("      <xsl:if test=\"" + value + "='" + option + "'\"><option selected=\"yes\">" + option + "</option></xsl:if>");
					printLine("      <xsl:if test=\"" + value + "!='" + option + "'\"><option>" + option + "</option></xsl:if>");
				}
	...
	public void writeDisplayHeader(String name, boolean isTop, ArrayList list, ArrayList param)
		throws Exception
	{
		String today = igpp.util.Date.now();
		...
		if(igpp.util.Text.isMatch(name, "Person")) {
			...
			if(isTop) {
				printLine("<tr><td><xsl:value-of select=\"ResourceID\"/></td><td width=\"50%\" align=\"right\"><xsl:value-of select=\"" + name + "\"/></td></tr>");
			} else {
		...
		printLine("<table>");
		printLine("   <xsl:if test=\"count(child::*)=0\"><tr><td colspan=\"2\"></td></tr></xsl:if>");	// Safegaurd from empty tags
	}

	public void writeDisplayFooter(String name, boolean isTop)
		throws Exception
	{
		printLine("</table>");
		printLine("</div>");
		...
		throws Exception
	{
		...
		for(Iterator i = list.iterator(); i.hasNext(); ) {
			...
			case TypeContainer:
				if(multiple) {
					printLine("   <tr><td colspan=\"2\"><xsl:call-template name=\"" + elemName + "\" /></td></tr>");
				} else {
					printLine("   <tr><td colspan=\"2\"><xsl:apply-templates select=\"" + elemName + "\" /></td></tr>");
				}
				break;
			case TypeCount:
			case TypeDate:
			...
			case TypeTime:
			case TypeEnumeration:
				print("   <xsl:if test=\"string-length(" + value + ") &gt; 0\">");	// Show if it has content
				print("   <tr><td valign=\"top\">");
				if(multiple) print("  <xsl:if test=\"position()=1\">");
				print(name + ":");
				if(multiple) print("</xsl:if>");
				print("   </td>");
				print("<td>");
				print(linkOpen);
	...
	public void writeDisplayItem(String object, String name)
		throws Exception
	{
		// Do nothing
	}
	...
	{
		return("\n"
						+ "      <xsl:if test=\"position()=1\">&nbsp;<a href=\"?new={$elemPath}/" + name + "\">new</a></xsl:if>\n"
						+ "      <xsl:if test=\"position()!=1\">&nbsp;<a href=\"?remove={$elemPath}/" + name + "[{position()-1}]\">remove</a></xsl:if>\n"
						+ "   "
				);
	}
	/**
	...
	**/
	 ...
	 **/
	public ArrayList makeEnum(String prefix, String list)
		throws Exception
	{
		String	query;
		Statement	statement;
		...
		while(resultSet.next())	{
			...
			if(enumName.length() != 0) enumList.addAll(makeEnum(buffer, enumName));
		}
		// Clean-up
	   this.endQuery(statement, resultSet);
	...
	public void makeAll(boolean forEdit)
		throws Exception
	{
