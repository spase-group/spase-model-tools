...
// Database access variables
	...
	public static void main(String args[])
   {
		...
		throws Exception
	{
		...
		printLine(1, "<xmi:Documentation xmi:Exporter='SPASE MakeXMI' xmi:ExporterVersion='" + mVersion + "' />");
	...
	**/
		...
		throws Exception
	{
		...
		printLine(1, "<ownedMember xmi:type='uml:DataType' name='int' visibility='public' xmi:id='Count'/>");
		printLine(1, "<ownedMember xmi:type='uml:DataType' name='DateTime' visibility='public' xmi:id='DateTime'/>");
		printLine(1, "<ownedMember xmi:type='uml:DataType' name='Duration' visibility='public' xmi:id='Duration'/>");
		printLine(1, "<ownedMember xmi:type='uml:DataType' name='double' visibility='public' xmi:id='Numeric'/>");
	...
	**/
		...
		throws Exception
	{
	   ...
	   resultSet.first();
		...
		do {
			...
			printLine(1, "<packagedElement xmi:type='uml:Enumeration' xmi:id='" + enumName + "' name='" + enumName + "' visibility='public'>");
			printLine(2, "<ownedComment xmi:type='uml:Comment' xmi:id='" + enumName + "_doc' body='" + igpp.util.Encode.htmlEncode(getElementDesc(name)) +"' >");
	 ...
	 * Generate XML schema description for enumerations
	 *
	 * @param prefix  context of the list.
		...
		throws Exception
	{
	   ...
	   if(prefix.length() > 0) prefix += ".";
		...
		do {
			...
			printLine(2, "<ownedLiteral xmi:type='uml:EnumerationLiteral' xmi:id='" + id + "' name='" + prefix + enumValue + "' visibility='public'>");
			printLine(3, "<ownedComment xmi:type='uml:Comment' xmi:id='" + id + "_doc' body='" + igpp.util.Encode.htmlEncode(getElementDesc(value)) +"' >");
		...
		throws Exception
	{
	   ...
	   if(rows == 0) {	// A leaf
	   	...
	   	className = getXMIName(term);
			...
			printLine(4, "<ownedComment xmi:type='uml:Comment' xmi:id='" + className + "_doc' body='" + igpp.util.Encode.htmlEncode(getElementDesc(term)) +"' >");
			...
			do {	// We're at the first record coming in to part of the code
				...
				printLine(4, "</ownedComment>");
				printLine(4, "<lowerValue xmi:id='" + id + "_lower_cardinality' value='" + lower + "' xmi:type='uml:LiteralInteger'/>");
				if(upper.compareTo("*") == 0) {
					printLine(4, "<upperValue xmi:id='" + id + "_upper_cardinality' value='" + upper + "' xmi:type='uml:LiteralUnlimitedNatural' />");
				} else {
					printLine(4, "<upperValue xmi:id='" + id + "_upper_cardinality' value='" + upper + "' xmi:type='uml:LiteralInteger'/>");
				}
				printLine(3, "</ownedAttribute>");

				if(isContainer(elemName)) {	// Draw association
					assocList.add(new String[] {className, name, lower, upper});
				}
			}	while(resultSet.next());
			printLine(2, "</packagedElement>");
		}
	   ...
	   for(int i = 0; i < assocList.size(); i++) {
	   	...
	   	upper = list[3];
			...
			printLine(2, "<packagedElement xmi:type='uml:Association' xmi:id='" + assocID + "'>");
				printLine(3, "<memberEnd xmi:idref='" + assocID + "_a" + "'/>");
				printLine(3, "<memberEnd xmi:idref='" + assocID + "_b" + "'/>");
				printLine(3, "<ownedEnd xmi:type='uml:Property'  xmi:id='" + assocID + "_b" + "' association='" + assocID + "' isNavigable='true' type='" + getXMIName(className) + "' visibility='public'>");
				printLine(4, "<lowerValue value='" + lower + "' xmi:id='" + assocID + "_b_lower_cardinality' xmi:type='uml:LiteralInteger'/>");
				if(upper.compareTo("*") == 0) {
					printLine(4, "<upperValue value='1' xmi:id='" + assocID + "_b_upper_cardinality' xmi:type='uml:LiteralInteger'/>");
				} else {
					printLine(4, "<upperValue value='1' xmi:id='" + assocID + "_b_upper_cardinality' xmi:type='uml:LiteralInteger'/>");
				}
   ...
   /* construct the association name */
	...
	public String getXMIType(String term)
		throws Exception
	{
	   String buffer = getElementType(term);
  		if(igpp.util.Text.isMatch(buffer, "container")) {
	...
	public String getXMIAggregation(String term)
		throws Exception
	{
	   String buffer = getElementType(term);
  		if(igpp.util.Text.isMatch(buffer, "container")) {
	...
	{
		if(igpp.util.Text.isMatch(type, "Count")) return "integer_id";
		if(igpp.util.Text.isMatch(type, "DateTime")) return "string_id";
		if(igpp.util.Text.isMatch(type, "Duration")) return "string_id";
		if(igpp.util.Text.isMatch(type, "Numeric")) return "double_id";
	...
	{
		occur = occur.trim();
		if(igpp.util.Text.isMatch(occur, "0")) return "0";	// Optional
	...
	{
		occur = occur.trim();
		if(igpp.util.Text.isMatch(occur, "0")) return "1";	// Optional
		if(igpp.util.Text.isMatch(occur, "1")) return "1";	// One only
